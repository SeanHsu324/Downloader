...
def download_video(url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var):
    try:
        if format_choice == "mp3":
            if cookie_check_button_var == "True":
                ydl_opts = {
                    'format': f"{mp3_format}",
...
def download_playlist(playlist_url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var):
    try:
        if format_choice == "mp3":
            if cookie_check_button_var == "True":
                ydl_opts = {
...
def download_channel_videos(channel_url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var):
    try:
...
def on_download_button_click(cook, url_box, dropdown_menu, ffmpeg_path, first_open, mp3_format, mp4_format, download_folder,check_button_var, cookie_check_button_var):
            ...
            if is_youtube_channel_url(url):
                if check_button_var == "True":
                    ...
                    if result:
                        ...
                        Thread(target=download_channel_videos, args=(url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var)).start()
                    else:
                ...
                Thread(target=download_playlist, args=(url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var)).start()
            else:
                if 'list=' in url:
                    url = url.split('&list=')[0]  # 移除播放清單參數，只下載單一影片
                Thread(target=download_video, args=(url, format_choice, progress_window, on_complete_callback, url_box, ffmpeg_path, mp3_format, mp4_format, download_folder, cookie_check_button_var)).start()
        else:
