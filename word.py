...
def start_conversion(wordlabel, wordstart_button, first_open):
    ...
    if first_open != 1:
        ...
        if file_path.endswith(".pdf"):
            ...
            threading.Thread(target=pdf_to_word(file_path, out_file, wordstart_button), daemon=True).start()
            wordlabel.configure(text="轉換中")
        elif file_path.endswith(".docx"):
            out_file = os.path.dirname(file_path)
            threading.Thread(target=word_to_pdf(file_path, out_file, wordstart_button), daemon=True).start()
            wordlabel.configure(text="轉換中")
        else:
