...
ffmpeg_path = resource_path("bin/ffmpeg/ffmpeg.exe")
...
if os.path.exists(json_file_path):
    with open(json_file_path, "r") as file:
        ...
        if "renew_check_button" not in settings:
            ...
            with open(json_file_path, "w") as file:
                json.dump(settings, file)
        if "cookie_check_button" not in settings:
            settings["cookie_check_button"] = "False"
            with open(json_file_path, "w") as file:
                json.dump(settings, file)
else:
...
def teaching(): 
    open_link_in_thread("https://sites.google.com/view/yt-to-dowload/%E6%95%99%E5%AD%B8/%E6%95%99%E5%AD%B8")
...
def cookie_choose_Check_value():
    if settings["cookie_check_button"]=="True":
        
            settings["cookie_check_button"] = cookie_status_var.get()
            with open(json_file_path, "w") as file:
                json.dump(settings, file)
    else:
        settings["cookie_check_button"] = cookie_status_var.get()
        ...
        cookie_txt_path = os.path.join(down_path, "cookie.txt")
...
def backhomepag():
    ...
    cookie_check_button.pack_forget()
...
def setepag():
    ...
    cookie_check_button.pack(side="top", anchor="w",pady=(10, 10))
...
yt_button = ctk.CTkButton(root, text="開始下載", width=100, height=40, corner_radius=40, command=lambda:on_download_button_click(root, url_box, dropdown_menu, ffmpeg_path, first_open, settings["mp3"], settings["mp4"], settings["download_folder"], settings["check_button"], settings["cookie_check_button"]), font=("Arial", 20, "bold"))
...
ToolTip(check_button, "啟用後將於下載全頻道影片時顯示確認提示")
...
cookie_status_var = ctk.StringVar(value=settings["cookie_check_button"])
cookie_check_button = ctk.CTkCheckBox(
    sett_menu_frame_right,
    text="使用Cookies",
    variable=cookie_status_var,
    onvalue="True",  # 勾選時的值
    offvalue="False", # 未勾選時的值
    command=cookie_choose_Check_value,
    font=("Arial", 16)
)
ToolTip(cookie_check_button, "啟用Cookies時，請先到官網了解使用方法")
